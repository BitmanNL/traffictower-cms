...
class grocery_CRUD_Model  extends CI_Model  {
    	...
    	if(!empty($this->relation))
    	{
    		foreach($this->relation as $relation)
    		{
				...
				if(strstr($related_field_title,'{'))
				{
    				...
    				$select .= ", TRIM(BOTH '&nbsp;' FROM REPLACE(CONCAT('".str_replace(array('{','}'),array("',COALESCE({$unique_join_name}.",", ''),'"),str_replace("'","\\'",$related_field_title))."'), '&nbsp;&nbsp;', '&nbsp;')) as $unique_field_name";
				}
    			else
    			{
    ...
    protected function relation_n_n_queries($select)
    {
    	...
    	foreach($this->relation_n_n as $relation_n_n)
    	{
	    	...
	    	if($use_template)
	    	{
	    		...
	    		$field .= "TRIM(BOTH '&nbsp;' FROM REPLACE(CONCAT('".str_replace(array('{','}'),array("',COALESCE(",", ''),'"),str_replace("'","\\'",$title_field_selection_table))."'), '&nbsp;&nbsp;', '&nbsp;'))";
	    	}
	    	else
	    	{
    ...
    {
        $this->db->group_by( $key, $value, $escape);
    }
    ...
    function get_relation_array($field_name , $related_table , $related_field_title, $where_clause, $order_by, $limit = null, $search_like = null)
    {
    	...
    	{
    		$related_field_title = str_replace(" ", "&nbsp;", $related_field_title);
    		$select .= "TRIM(BOTH '&nbsp;' FROM REPLACE(CONCAT('".str_replace(array('{','}'),array("',COALESCE(",", ''),'"),str_replace("'","\\'",$related_field_title))."'), '&nbsp;&nbsp;', '&nbsp;')) as $field_name_hash";
    	}
    	else
    	{
    ...
    function get_relation_n_n_selection_array($primary_key_value, $field_info)
    {
        ...
        if(!empty($field_info->relation_where_clause)){
            $this->db->where($field_info->relation_where_clause);
        }

        if(!empty($field_info->where_clause)){
    	...
    	if(!empty($post_data))
    	{
    		foreach($post_data as $primary_key_value)
	    	{
				...
				if($count == 0)
				{
                    ...
                    if(!empty($field_info->relation_where_clause)){
                        $where_array = array_merge($where_array, $field_info->relation_where_clause);
                    }
