...
$(function(){
	...
	$('#filtering_form').submit(function(){
		...
		$(this).ajaxSubmit({
			 ...
			 success:    function(data){
			 	...
			 	// start at page 1 if total number of results don't match current
			 	if (parseInt($('#total_items').html(), 10) !== parseInt(data.total_results, 10)){
			 		$('#tb_crud_page').val('1');
	...
	$(document).on('click', '.field-sorting', function(){
	...
	$(document).on('click', '.delete-row', function() {
		...
		CMS.modal({
			...
			callBackSubmit: function(){
				deleteGroceryCrudInformation($(that).data('target-url'));
			}
		});
		return false;
	});
...
function deleteGroceryCrudInformation(delete_url){
	$('#ajax-loading').removeClass('hide');
	$.get(delete_url, function(data){
		if(data.success) {
			$('#filtering_form').trigger('submit');
			CMS.alert(data.success_message.replace('<p>', '').replace('</p>', ''), 'success');
		} else {
			CMS.alert(data.error_message.replace('<p>', '').replace('</p>', ''));
		}
	}, 'json');
	$('#ajax-loading').addClass('hide');
	return true;
