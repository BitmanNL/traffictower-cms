				...
				if(!isset($types[$field_name]))//Doesn't exist in the database? Create it for the CRUD
				{
					...
					$field_info = (object)array(
						...
						'display_as' => isset($this->display_as[$field_name]) ?
												$this->display_as[$field_name] :
												ucfirst(str_replace("_"," ",$field_name)),
						'required'	=> (is_array($this->required_fields) && in_array($field_name,$this->required_fields)) ? true : false,
						'extras'	=> $extras
					);
...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	{
		$ci = &get_instance();
		$ci->load->model('admin/grocery_CRUD_Model');
	...
	protected function set_ajax_list_queries($state_info = null)
	{
		...
		if(!empty($state_info->search))
		{
			...
			if($state_info->search->field !== null)
			{
				...
				{
					$new_column = $this->repair_column_name($state_info->search->field, $this->basic_model->get_table_name(), FALSE);
					$this->like($new_column , $state_info->search->text);
				}
			}
			else
			{
				...
				foreach($columns as $column)
				{
					...
					elseif(isset($this->relation_n_n[$column->field_name]))
					{
						...
						if (is_array($temp_relation_n_n->relation_where_clause))
						{
							foreach ($temp_relation_n_n->relation_where_clause as $where_field => $where_value)
							{
								if ($where_value === NULL)
								{
									$join_where .= ' AND `' . $temp_relation_n_n->relation_table . '`.`' . $where_field . '` IS NULL';
								}
								else
								{
									$join_where .= ' AND `' . $temp_relation_n_n->relation_table . '`.`' . $where_field . '` = ' . $ci->db->escape($where_value);
								}
							}
						}
				...
				if (!empty($where))
				{
					...
					$this->group_by($this->basic_model->get_table_name() . '.' . $this->basic_model->get_primary_key());
				}
			}
		}
	}
	...
	protected function get_list()
	{
		...
		if(!empty($this->group_by))
			foreach($this->group_by as $group_by)
				$this->basic_model->group_by($group_by[0],$group_by[1],$group_by[2]);
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_integer_input($field_info,$value)
	{
		...
		$input = "<div class='row'><div class='col-md-4'><input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$value' class='numeric form-control' $extra_attributes /></div></div>";
		return $input;
	}
	...
	protected function get_string_input($field_info,$value)
	{
		...
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
		$input = "<div class='row'><div class='col-md-6'><input id='field-{$field_info->name}' name='{$field_info->name}' type='text' class=\"form-control\" value=\"$value\" $extra_attributes /></div></div>";
		return $input;
	}
	...
	protected function get_text_input($field_info,$value)
	{
		if($field_info->extras == 'text_editor')
		{
			...
			switch ($editor) {
				...
				case 'tinymce':
					$this->set_js_lib($this->default_texteditor_path.'/tinymce4/tinymce.min.js');
		...
		{
			$input = "<textarea id='field-{$field_info->name}' name='{$field_info->name}' class='form-control' rows='5'>$value</textarea>";
		}
		return $input;
	}
	...
	{
		$this->set_css($this->default_css_path.'/jquery_plugins/jquery.datetimepicker.css');
		$this->set_js_lib($this->default_javascript_path.'/jquery_plugins/jquery.datetimepicker.js');
		...
		$input = "<div class='row'><div class='col-md-4'><input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$datetime' maxlength='19' class='datetime-input form-control' /></div></div>";
		return $input;
	}
	...
	protected function get_password_input($field_info,$value)
	{
		...
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
		$input = "<div class='row'><div class='col-md-4'><input id='field-{$field_info->name}' name='{$field_info->name}' type='password' value='$value' $extra_attributes class='form-control' /></div></div>";
		return $input;
	}
	...
	protected function get_date_input($field_info,$value)
	{
		...
		$input = "<div class='row'><div class='col-md-4'><input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$date' maxlength='10' class='date-input form-control' /></div></div>";
		return $input;
	}
	...
	protected function get_dropdown_input($field_info,$value)
	{
		...
		$input = "<div class='row'><div class='col-md-4'><select id='field-{$field_info->name}' name='{$field_info->name}' class='chosen-select form-control' data-placeholder='".$select_title."'>";
	...
	protected function get_set_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<div class='row'><div class='col-md-4'><select id='field-{$field_info->name}' name='{$field_info->name}[]' multiple='multiple' size='8' class='chosen-multiple-select form-control' data-placeholder='$select_title' style='width:510px;' >";
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $default_language_path				= 'assets/admin/grocery_crud/languages';
	protected $default_config_path					= 'assets/admin/grocery_crud/config';
	protected $default_assets_path					= 'assets/admin/grocery_crud';
...
//@todo ui_day, ui_month, ui_year has to be lang strings
	...
	public function group_by($key, $value = NULL, $escape = TRUE)
	{
		$this->group_by[] = array($key,$value,$escape);
	 ...
	 * @param mixed $where_clause
	 */
	public function set_relation_n_n($field_name, $relation_table, $selection_table, $primary_key_alias_to_this_table, $primary_key_alias_to_selection_table , $title_field_selection_table , $priority_field_relation_table = null, $where_clause = null, $relation_where_clause = null)
	{
		$this->relation_n_n[$field_name] =
			(object)array(
				...
				'relation_where_clause' => $relation_where_clause
			);
